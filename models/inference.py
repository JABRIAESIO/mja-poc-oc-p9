...
def preprocess_image(image, target_size=(224, 224)):
    """
    Prétraite une image pour l'inférence avec le modèle EfficientNetB0-Transformer.
    
    Args:
        image: Image PIL
        target_size: Taille cible (défaut: 224x224 pour EfficientNetB0)
        
    Returns:
        Tableau numpy normalisé prêt pour l'inférence
    """
    # Redimensionner et padder l'image
    ...
    if len(img_array.shape) != 3 or img_array.shape[2] != 3:
        raise ValueError(f"L'image doit avoir 3 canaux (RGB). Forme actuelle: {img_array.shape}")
...
def predict_image(model, image, categories, verbose=True):
    """
    Prédit la classe d'une image avec le modèle EfficientNetB0-Transformer.
    
    Args:
        model: Modèle Keras chargé avec les classes personnalisées
        image: Image PIL
